...
if isvector(ydata)
    ...
    for idx = 1:ngrp
        ...
        default = true;
        for i = 1:2:numel(args)
            if startsWith('Orientation',args{i},'IgnoreCase',true) && startsWith('horizontal',args{i+1},'IgnoreCase',true)
                default = false;
            end
        end
        rulerSwap = true;
        ...
        H(idx) = matlab.graphics.chart.primitive.BoxChart('Parent', cax,...
            ...
            'XDataMode', xdatamode, 'PeerID', idx,'IsRulerSwap',rulerSwap, args{:}, grpargs{:});
