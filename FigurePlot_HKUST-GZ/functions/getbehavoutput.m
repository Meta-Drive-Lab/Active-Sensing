...
item1chosen_valdiff_lastRV = nan(length(dstruct),length(valdiffunique));
item1chosen_valdiff_lastFV = nan(length(dstruct),length(valdiffunique));
...
for s = 1:length(dstruct)
    ...
    for i = 1:length(valdiffunique)
        ...
        if any(v_i)
            ...
            item1chosen_valdiff_lastRV(s,i) = mean(dstruct(s).choice(v_i & lastfixitem==1)==1);
            item1chosen_valdiff_lastFV(s,i) = mean(dstruct(s).choice(v_i & lastfixitem==2)==1);
            % Normalized choices
...
outputstruct.item1chosen_valdiff_lastRV = item1chosen_valdiff_lastRV;
outputstruct.item1chosen_valdiff_lastFV = item1chosen_valdiff_lastFV;
